...
// 					// 	for (auto& paramStx : paras_ptr->stxs) {
// 					// 		SymbolSyntax* sym = dynamic_cast<SymbolSyntax*>(paramStx.get());
// 					// 		if (!sym) throw RuntimeError("lambda parameter must be a symbol");
// 					// 		// 修复：不要 parse！不要查环境！
// 					// 		vars.push_back(sym->s);
// 					// 	}
// 					//
// 					// 	// Body 用原始 env 解析（parse 阶段不依赖运行时环境）
// 					// 	if (stxs.size() == 3) {
// 					// 		Expr body = stxs[2]->parse(env);
// 					// 		return Expr(new Lambda(vars, body));
// 					// 	} else {
// 					// 		std::vector<Expr> body_exprs;
// 					// 		for (size_t i = 2; i < stxs.size(); i++) {
// 					// 			body_exprs.push_back(stxs[i]->parse(env)); // ✅ 用 env，不是 new_env
// 					// 		}
...
// 				case E_LAMBDA:{
// 					if (stxs.size() < 3) {
// 						throw RuntimeError("wrong parameter number for lambda");
// 					}
// 					Assoc New_env = env;
// 					std::vector<std::string> vars;
// 					List* paras_ptr = dynamic_cast<List*>(stxs[1].get());
